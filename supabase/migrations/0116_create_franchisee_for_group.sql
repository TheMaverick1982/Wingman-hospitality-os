-- Let a FRANCHISOR admin create a new franchisee account inside their own group
-- (self-service invite). create_organization_for_user() can't be reused: it
-- requires the caller to be a PLATFORM admin. This authorizes on franchise-admin
-- membership instead, seeds the same starter content, and auto-attaches the new
-- org to the group with the group's billing mode.

-- Shared starter-content seed (same defaults a brand-new org gets). Extracted so
-- the franchisee path and the platform path stay identical over time.
create or replace function seed_org_defaults(new_org_id uuid)
...
create or replace function create_franchisee_for_group(
  group_id uuid,
  org_name text,
  owner_user_id uuid,
  gm_full_name text,
...
declare
  ...
  is_central boolean;
begin
  -- Caller must be an ADMIN of this franchise group.
  if not exists (select 1 from franchise_admins where group_id = create_franchisee_for_group.group_id and user_id = auth.uid() and role = 'admin') then
    raise exception 'Only a franchisor admin can add a franchisee to this group';
  end if;
  ...
  select billing_mode = 'central' into is_central from franchise_groups where id = create_franchisee_for_group.group_id;

  -- Franchisees are real paying accounts (central groups are covered via the
  -- group roll-up, tracked by billed_by_group).
  insert into organizations (name, is_free_account, franchise_group_id, billed_by_group)
    values (org_name, false, create_franchisee_for_group.group_id, coalesce(is_central, false))
  ...
  insert into franchise_memberships (group_id, org_id, status)
    values (create_franchisee_for_group.group_id, new_org_id, 'active')
    on conflict (org_id) do update set group_id = excluded.group_id, status = 'active';

  perform seed_org_defaults(new_org_id);
