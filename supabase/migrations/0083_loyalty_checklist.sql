-- FOH loyalty checklist. Reuses the existing accountability checklist model:
-- an editable per-org template ('loyalty' type) plus per-staff completions in
-- shift_checklist_completions (whose checklist_type is free text, so no change
-- is needed there). Only the checklist-item template's type constraint needs
...
alter table accountability_checklist_items
  ...
  check (checklist_type in ('daily', 'preshift', 'loyalty'));
