...
import { ContactPanel, type ContactRecord, type ActivityRecord } from "./contact-panel";
...
export default async function CrmContactPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: contact } = await admin
    .from("crm_contacts")
    .select("id, email, name, phone, stage, first_source, unsubscribed, org_id, created_at")
    .eq("id", id)
    .maybeSingle();
  if (!contact) notFound();
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <ContactPanel contact={contact as ContactRecord} activities={(activities ?? []) as ActivityRecord[]} />
    </div>
  );
}
