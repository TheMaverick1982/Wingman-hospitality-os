...
export async function GET(request: NextRequest) {
  ...
  const { data: rows, error } = await admin
    ...
    .select("id, location_id, due_at, status, last_score, staff_members(full_name), tests(title)")
  ...
  // Cache location email lookups within this run.
  ...
  for (const raw of rows ?? []) {
    const a = raw as unknown as {
      id: string; location_id: string | null; status: string;
