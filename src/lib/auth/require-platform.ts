...
import { canAccessPlatformSection, type PlatformPermission } from "./platform";
...
export async function requirePlatformSection(section: PlatformPermission): Promise<CurrentProfile> {
...
// permission, else null (the caller returns an error rather than redirecting).
export async function platformSectionActor(section: PlatformPermission): Promise<CurrentProfile | null> {
  const profile = await getCurrentProfile();
