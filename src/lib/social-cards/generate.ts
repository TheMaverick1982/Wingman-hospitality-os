...
export async function generateSocialDrafts(count: number): Promise<{ error: string | null; created: number }> {
...
- "link" is one of the real CTA URLs and should vary across the batch.`;
  ...
  for (let i = 0; i < ideas.length && i < slots.length; i++) {
    ...
    if (idea.facebook?.caption) rows.push({ caption: idea.facebook.caption.trim(), link, first_comment: idea.facebook.first_comment?.trim() || null, platforms: ["facebook"], format: "feed", scheduled_at: slot, image_paths: img, card_text: cardText, status: "draft" });
    if (idea.instagram?.caption) rows.push({ caption: idea.instagram.caption.trim(), link: null, first_comment: idea.instagram.first_comment?.trim() || null, platforms: ["instagram"], format: "feed", scheduled_at: slot, image_paths: img, card_text: cardText, status: "draft" });
    if (idea.linkedin?.caption) rows.push({ caption: idea.linkedin.caption.trim(), link, first_comment: idea.linkedin.first_comment?.trim() || null, platforms: ["linkedin"], format: "feed", scheduled_at: slot, image_paths: [], status: "draft" });
    created++;
  }
