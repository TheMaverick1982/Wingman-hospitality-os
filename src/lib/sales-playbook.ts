...
export const PRODUCT_TOUR: ProductArea[] = [
  ...
  { name: "Data safety & recovery", what: "Deleting a guest, partner, or staff member is a soft delete — it moves to an owner-only Trash where it can be restored exactly as it was, or removed for good. A recent-activity audit log records every delete, restore, and permanent removal with who did it and when, and the whole account is backed up at the database level as a separate catastrophe net.", problem: "Owners worry a disgruntled or careless employee could wipe their data. This makes deletions reversible and traceable without a support ticket — 'nobody can quietly destroy your account' is a real peace-of-mind close, especially for multi-location operators handing logins to managers." },
  { name: "Direct POS integrations", what: "Connect Square in one click from Settings → API access → Direct integrations — no API key or Zapier. Wingman then auto-syncs the merchant's customers into Guest Bounce Back (deduped by email/phone) and the last 7 days of sales into the weekly Business Health card, matched per location by store name for multi-location operators — once a day or on-demand with 'Sync now'. Clover, Toast, and Global Payments are being added to the same list. (A code/Zapier API path also exists for everyone else.)", problem: "The #1 friction in restaurant software is data entry. A one-click POS connection means their guests and numbers populate themselves — no key, no Zap, no manual import — so the dashboard and win-back lists are live from day one. 'It just works with your Square' is a powerful close, and per-location matching lands with multi-unit operators." },
];
