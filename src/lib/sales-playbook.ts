...
export const PRODUCT_TOUR: ProductArea[] = [
  ...
  { name: "Training & Standards", what: "Builds role-by-role training programs with a real sign-off log, plus a Tests & exams area: auto-scored multiple-choice / true-false tests built by AI, from a role's own training, or from ready-made Food/Bartender examples — with owner-set pass %, retakes, multi-day pacing, deadlines, role targeting, and a learn-then-quiz mode for a new LTO/menu that can rotate monthly to all staff. Assign a test to a person, a role, or everyone (they're emailed a link), they take it a day at a time, it auto-scores, and a results board shows who passed and who hasn't. If someone burns all their retakes it locks and emails the location's manager to coach and unlock; if a deadline passes unfinished, the manager gets an overdue nudge too — both routed to the location's email on file.", problem: "New hires 'pick it up as they go,' so the experience drifts — and owners have no proof anyone actually learned the standards. This makes training consistent AND verifiable: you can require a passing score before someone works the floor or the bar, and managers get pulled in exactly when someone's struggling instead of finding out too late." },
...
export const MOVEMENTS: Movement[] = [
  ...
  },
  {
    ...
    script: [
      ...
      "\"And here's how you prove they learned it, not just sat through it.\" — open Training → Tests, show the Food Test broken out by menu category, then open \"Assigned to you\" and take a question or two so they see the exact staff experience. Point out the results board (who passed, who's locked) and that a lock or a missed deadline emails the manager automatically. (The demo account is pre-loaded with all of this.)",
    ],
    doThis: [
