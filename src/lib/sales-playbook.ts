...
export const SYSTEM_REFERENCE: SystemNote[] = [
  { name: "Staff View toggle (demo, internal)", detail: "In the demo, a bar at the top lets the rep flip between the owner view and a Staff View — one click, mid-call — to show exactly what a team member sees: their pre-shift checklist, an assigned test to take, their training, and the pre-shift focus / weekly experiment on Culture, with the owner-only sections (Reporting, Growth, Hiring, Settings, etc.) hidden from the nav. Flip back to owner view just as easily. It's demo-only (never shown on a real customer account) and changes only what's displayed, not the data. Use it to answer the common 'what will my staff actually use?' objection by SHOWING the focused, stripped-down staff experience instead of describing it." },
