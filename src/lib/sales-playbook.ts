...
export const PRODUCT_TOUR: ProductArea[] = [
  ...
  { name: "Hiring, Growth & Menu", what: "Interview criteria that screen for the person, plus a ready-made online application form (shareable link or embed on their own site, no website or subdomain needed, pre-fillable by role/location) that feeds an Applicants tracker — status (new/contacted/not a fit/hired), schedule-a-visit, optional resume, and a one-click path into a scored candidate. Plus revenue and menu-engineering tools.", problem: "Rounds out the system — they collect applications without paying for a careers page or a job board, every applicant lands in one tracked pipeline instead of a messy inbox, and better people flow straight into the interview scorecard. Great for the owner drowning in DMs and texts from applicants." },
