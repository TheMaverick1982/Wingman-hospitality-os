...
export const SYSTEM_REFERENCE: SystemNote[] = [
  ...
  { name: "Lean navigation (progressive disclosure)", detail: "The sidebar defaults to a short, calm list — the sections a restaurant uses most are always visible (Dashboard, Guest Bounce Back, Hiring, Training & Standards, Accountability, Staff, Culture), and everything else is tucked under a single 'More' expander (Guest Reviews, Service Recovery, Guest Journey, Shift, Manager channel, Questions, Revenue Growth Planner, Menu Engineering, Standout Audit, Partners, Reporting). 'More' remembers whether it's open and auto-opens when you're on one of those pages, so nothing is ever hard to reach — it's just not all shoved in your face on day one. Hospitality Score now lives inside Reporting (one link at the top of that page) rather than as its own nav item. This is the answer to 'isn't there a lot here?' — there's a lot of POWER, but the default view is deliberately simple, and per-manager section hiding (below) trims it further per person. Demo note: point out how short the sidebar is, then click 'More' to reveal the depth.", },
