...
export const PRODUCT_TOUR: ProductArea[] = [
  ...
  { name: "Guest Reviews (survey)", what: "A guest-experience survey with a per-location QR code + short branded link (joinwingman.app/s/…) — put it on a table tent, receipt, or window, or text/email it. Guests rate food, service, and likelihood to return, can name who served them (first names only, for a shout-out), and leave a comment. Responses land in a Guest Reviews archive with response count, average rating, and rave-review count, filterable by location, and each location's link tracks scans. Deliberately SEPARATE from Guest Bounce Back — a survey never adds a guest or counts as a visit, so repeat-rate data stays clean — with a match-ONLY link to a guest who's already in Bounce Back. A 'How guests are feeling' card sits on EVERYONE's dashboard (staff included) with the average rating and positive shout-outs that name the server by first name, and managers get a one-click AI readout at the top of the page — what guests love, where to improve, and the single highest-leverage fix to make this week — grounded in the restaurant's own Owner's Mindset. That readout is ONE combined read across BOTH the survey feedback AND the connected Google reviews, so the owner gets the whole picture in a single place instead of two separate analyses. NEW — it can auto-send: the owner flips it to Weekly or Monthly and Wingman emails each location's combined report to that location's managers + the owner (Mondays, or the 1st), so the feedback pulse lands in their inbox without opening the app — per-location for multi-unit groups, plus an optional master copy address (head office / a regional manager) that receives every location's report. NEW — Ownership recap: a SEPARATE, owner-only channel that emails ONE company-wide read across ALL locations combined to specific ownership addresses, daily (every morning) or weekly — what guests love and where to improve company-wide, with a few supporting direct quotes pulled verbatim from the feedback, and an optional 'This week' action the owner can switch off for a pure highlights view. Only a Super Admin can see it or edit the recipient list, so it's the exec/ownership rollup that sits above the per-location manager reports. The 'who took care of you?' question is optional: a one-switch toggle on the Guest Reviews page turns the server picker off account-wide (for counter-service or owners who don't want reviews tied to a person), and a per-person 'Show on the guest survey' toggle on each staff profile hides just one individual (marketing, catering, back office) while keeping the question. NEW — Google reviews: connect the restaurant's Google Business Profile (official read-only access — Wingman never posts) and it pulls each location's real Google reviews, then the AI reads them per location — rating + trend, what guests love, where they're slipping (the recurring complaints), the themes, and concrete actions to raise the rating; refreshes weekly, and each location links to its own Google listing so multi-unit groups get a per-store read. Multi-location is first-class: from the 'All locations' view the owner sees every store at once and links each to its Google listing without switching stores, and 'Connect another account' handles chains whose locations live under different Google logins (with a full 'Disconnect Google' to reset). So the same page now holds BOTH first-party survey feedback AND their public Google reputation, analyzed side by side. NEW — review→shout-out bridge: when a survey names a server, managers get a one-tap 'Recognize [name]' button under that review that posts it straight to the Culture Wins feed — carrying the guest's own words — so real guest praise becomes team recognition without retyping it (idempotent, and it flips to 'Recognized' once posted). That closes the loop between the guest side and the culture side of the product on a single tap.", problem: "Operators fly blind on how guests actually felt until a one-star Google review shows up days later. This captures first-party feedback in the moment, at the table, per location — problems surface fast, wins get seen — without polluting the retention numbers. It also supplies the guest-experience half of the retention story the whole product is built around, and doubles as a recognition engine when a guest calls out their server by name." },
  ...
  { name: "Culture", what: "Captures the owner's standard, core values, and weekly focus in their own words — anchored by the Owner's Mindset: a short 'run it like you own it' manifesto (imagine your own savings are on the line; every guest is why the doors stay open) that the owner writes or drafts with AI, every hire reads at the top of their role guide, and that grounds the Ask Wingman assistant and all AI generation. NEW — core values are fully editable: owners/managers add, rename, reword, reorder, or remove them right on the Culture page (they come pre-filled with a strong starting set); they show on every role guide, ground the AI, and double as the universal hiring criteria candidates are screened against — so belief (values) connects straight to hiring behavior. NEW — weekly experiment with a CLOSED LOOP: the owner sets one small test to run this week (a new upsell, a new table-side line), and when it's done records the result — It worked / Mixed / No change plus a note on what they learned. That archives it into an Experiment log (a running history of every bet and how it turned out) and clears the field for the next one — so the weekly experiment stops being a write-once-and-forget field and becomes an accumulating record of what actually moves the needle in THAT restaurant. NEW — Monthly Culture Recap: the owner flips one switch and on the 1st of every month Wingman AI-writes a one-page recap of how the culture actually showed up — the month's recognition (how much, who stood out, which core values appeared and which went quiet), the anonymous team pulse WITH its trend vs last month, and the experiments run and how they turned out — and emails it to the owner + managers. A 'Preview recap' button shows it on demand. It turns all the culture signals the product already captures into a single leadership read that lands in the inbox, so culture gets a monthly moment of attention instead of being something you only look at when someone quits. It's the culture core the rest of the product hangs on.", problem: "Culture is felt but never written down, so it can't be trained or scaled to new hires and second locations — and 'take pride in your work' is too vague to change behavior. The Owner's Mindset turns culture into a concrete, ownership-framed standard every role feels and every shift is held to, and because it feeds the AI, that mindset shows up in the training, role guides, and answers staff actually see. The weekly experiment loop adds a habit of continuous improvement most restaurants never build — small bets, tracked, so owners keep what works and drop what doesn't instead of guessing." },
  { name: "Wins feed (team recognition)", what: "Team-wide recognition built into Culture: any team member — not just managers — shares a win (a smooth shift, a big table handled) or shouts out a teammate by name, and it appears in a Wins feed everyone sees on the Culture page and on a 'Recent wins' card on every dashboard (staff included). Anyone can tap 🎉 Celebrate to pile on, and a 'Most recognized' board ranks the teammates getting the most shout-outs. Posts are self-attributed (you post as yourself) and managers can moderate. NEW — recognition ties to the org's OWN core values: when you post, you pick which of the owner's defined core values the win reflects (not a generic hardcoded tag), so shout-outs roll up to the handful of things the restaurant actually stands for. A 'Values in action' card on the Culture page then shows how often each core value is showing up in recognition — a live read on which values are living on the floor and which are being quietly ignored. (Orgs that haven't set values yet fall back to a simple tag.) NEW — it buzzes the team's PHONES: when anyone posts a win or shout-out, everyone on the team who has the app installed gets a push notification, so recognition is felt in the moment instead of scrolled past later — and setting the weekly pre-shift focus or experiment pushes the whole team too, so everyone starts the week pointed at the same thing (opt-in — OFF by default so no one is buzzed until the owner turns it on under Settings → Notifications → Culture). This is what makes recognition a live, felt thing across the team rather than a wall you have to remember to check. NEW — recognition can now START from a guest review: when a guest survey names a server, a manager taps 'Recognize [name]' on the Reviews page and the guest's praise lands here in the Wins feed automatically, so the best source of recognition (the guest) feeds the team's recognition habit directly.", problem: "Recognition in most restaurants is top-down, sporadic, and forgotten by the next shift — so the people quietly carrying the floor never feel seen, which is a top driver of turnover. This makes recognition a daily, bottom-up team habit that lives where everyone already looks, turns a good moment into something the whole crew celebrates, and gives owners a live read on who their culture-carriers actually are — AND on whether the values they wrote down are actually showing up in how the team recognizes each other, closing the loop between the values on the wall and behavior on the floor." },
  { name: "Team Culture Pulse (anonymous)", what: "A 30-second, fully anonymous monthly check-in every team member takes right on the Culture page: do they feel recognized, are the values clear, are they proud to work here — plus an optional comment. Owners/managers see a 'Team pulse' card with this month's averages (out of 5), the change vs last month, response count, and the anonymous comments. Anonymity is real and architectural: Wingman stores WHO checked in (to prevent double-submits) in a separate table from WHAT they said, so a manager can never trace a comment or score back to a person.", problem: "Owners find out someone was unhappy when they quit — too late. Every other Wingman metric reads the guest side or the owner's self-assessment; this reads the TEAM's side, which is the earliest leading indicator of turnover. A dip in 'feel recognized' or 'proud to work here' this month is a churn warning you can act on now — and because it's anonymous, you get the truth, not what people think you want to hear." },
  { name: "Training & Standards", what: "Builds role-by-role training programs with a real sign-off log, plus a Tests & exams area: auto-scored multiple-choice / true-false tests built by AI, from a role's own training, or from ready-made Food/Bartender examples — with owner-set pass %, retakes, multi-day pacing, deadlines, role targeting, and a learn-then-quiz mode for a new LTO/menu that can rotate monthly to all staff. NEW — one-click setup: a \"Set up a test for every role\" card on the Tests page writes a ready-to-assign, auto-scored test for EVERY role at once (from each role's own standards), with live progress — instead of building them one at a time; it skips any role that already has a test, so nothing an owner customized is overwritten, and the card disappears once every role is covered. It never stalls: if the hourly AI limit is reached partway through a big account, Wingman finishes the remaining roles by building those tests straight from each role's own standards (a fair, auto-scored quiz), and tells the owner which ones to enrich with AI later via \"Update the test\" — so setup always completes in one pass. Great for a brand-new account. Any role's training becomes a test in one click — \"Turn into a test\" sits right under the role's training program, shows a live \"building your test…\" indicator while the AI writes it, then hands you a \"Review & edit\" step so you read it over before assigning anyone (and \"Update the test\" regenerates it when the training changes). In the editor you can add or remove days on the fly — each day carries its own learning section plus its own questions — and trainees can work ahead through the days in one sitting. A one-click Preview (next to each test, and at the top of the test's page) walks the owner through the test exactly as staff see it — day by day, learning then questions, nothing saved — with a manager-only \"Show answer key\" toggle for reviewing correctness. The front door for handing them out is a prominent \"Start a test\" button on the Training page: pick who takes it (one person, a role, or all staff), check off the test(s) — hand out several at once — set an optional due date, and send. Everyone's emailed a link (one email even for multiple tests), they take it a day at a time, it auto-scores, and a results board shows who passed and who hasn't. If someone burns all their retakes it locks and emails the location's manager to coach and unlock; if a deadline passes unfinished, the manager gets an overdue nudge too — both routed to the location's email on file. And because hospitality is a habit, not a one-time course, there's a dedicated Continuing Education area (Training → 'Make great hospitality a habit'): a guided AI builder asks a couple of sharpening questions (what slipped, the reaction guests should have, the standard to hold) and offers role-aware idea chips, then drafts a short learn-then-quiz 'day one' the owner reviews and edits. On the 1st of every month Wingman re-assigns each refresher to the roles it targets as a fresh attempt and emails everyone a link — a core hospitality refresher for all staff, plus role-specific add-ons (a bartender's drink specs, a line cook's plating) that only reach that department — all with the same reminders and results tracking.", problem: "New hires 'pick it up as they go,' so the experience drifts — and owners have no proof anyone actually learned the standards. This makes training consistent AND verifiable: you can require a passing score before someone works the floor or the bar, and managers get pulled in exactly when someone's struggling instead of finding out too late." },
  ...
  { name: "Hiring, Growth & Menu", what: "Interview criteria that screen for the person, plus a ready-made online application form (shareable link or embed on their own site, no website or subdomain needed, pre-fillable by role/location) that the owner can fully customize — turn built-in fields on/off, rename and require them, and add their own custom questions (short text, paragraph, dropdown, yes/no, number), with answers shown right on each applicant's card. NEW — AI pre-interview SCREENING: per role, Wingman drafts a few short questions (3 guest-experience scenarios + 1 that quietly tests whether they follow directions) grounded in that role's hiring criteria, owner-editable, shown on the form once the candidate picks the role. When someone applies, Wingman grades their written answers and drops a read on the applicant's card — a Strong fit / Worth a look / Probably pass tier plus a 1–5 score for guest-experience instinct and for following instructions, each with a one-line reason — so the operator decides who's worth an interview BEFORE spending a slot. It's decision support (never auto-rejects, never changes status; the manager always decides) and the candidate-facing form just says 'we review every application and reach out if we'd like to interview' — no mention of AI. This all feeds an Applicants pipeline that flows in one direction: Applications (status new/contacted/not a fit, optional resume) → schedule + confirm an interview (date, time, details) → the person moves into the candidates area under 'Interviews scheduled' → score them after the interview → candidate. Any location with an interview booked that day gets a morning reminder email at its address on file. NEW — interview confirmation email: when a manager books an interview, the APPLICANT is automatically emailed the confirmed date, time, and location plus a 'call us if anything changes' note — from the restaurant's name, replies routed to the location (never Wingman), with a per-booking 'email the applicant' checkbox to skip it and a '✓ Invite emailed' marker. Owner-editable copy (its own 'Interview invitation email' section on the Hiring page, {{first_name}}/{{restaurant}}/{{role}}/{{date}}/{{time}}/{{location}}/{{address}}/{{phone}}/{{details}} placeholders, plus 'send test to me') — so applicants get a professional confirmation instead of a vague verbal time, cutting no-shows. NEW — one-click applicant replies: from any applicant's card the operator can send a warm 'we're interested, we'll reach out soon' note or a gracious 'not a good fit at the moment' note, so people who apply actually hear back instead of vanishing into a black hole — sent from the restaurant's own name, and replies route to the LOCATION's email on file (not Wingman), with a '✓ Sent' marker so no one is emailed twice. Sending 'interested' marks them Reviewed; sending 'not a fit' archives them. Both emails address the applicant by first name (junk/offensive names are filtered — falls back to 'Hi there'), and the operator can EDIT the wording of both in an 'Applicant reply emails' section on the Hiring page ({{first_name}}/{{restaurant}}/{{role}} placeholders, reset-to-default any time). A small touch that protects the operator's reputation and keeps good candidates warm. Plus revenue and menu-engineering tools — Menu Engineering plots each dish as a Star/Plowhorse/Puzzle/Dog by profit and popularity, and a whole menu can be brought in at once via CSV import with mappable columns (leave any field blank and fill it in later by editing the item). NEW — Job openings: post a specific role at a location (or all locations), let AI write the full job ad grounded in that role's own standards, duties, and hiring criteria (or paste an existing ad to sharpen), and get a short branded link (joinwingman.app/j/…) plus a QR code to post on Indeed/Craigslist/social or print on a window sign — applicants land pre-set to the role and location and are tagged to that opening, and Wingman counts clicks vs applications per posting (e.g. 120 clicks → 9 applied) so the operator sees which board actually works — analytics a plain TinyURL can't give; close/reopen/delete anytime, applicants are always kept. Hiring the same role across several stores? Tick multiple locations when creating and Wingman makes one opening per store (shared ad, separate links) instead of a single blurry 'all locations' post — and if the ad names a specific location, each store's posting is rewritten to its OWN name (no more every posting saying the location you created it from); and the type can be Full-time AND Part-time, not just one. Role not in the standard list? Pick '+ Custom role' and type anything (Baker, Valet, Event Lead) — AI still writes the ad and the custom name carries through the posting, careers page, and apply form — and each custom role gets its own tab in Screening questions, so you can author role-specific pre-interview screening for it just like a standard role. NEW — CORPORATE openings: a role that isn't at a restaurant (a corporate marketer, an HQ bookkeeper, a regional manager) can be posted as 'Corporate (no location)' — no store to pick. It's labeled 'Corporate' on the careers page (not the misleading 'All locations'), and applicants applying to it aren't shown or asked for a location. And because a corporate role has no store email to route to, the opening carries its own 'Send application alerts to' field — set an address (HR, a regional director) and new applications for that post email straight there, on top of any account-wide copy addresses. Answers 'how do I post a corporate job that isn't tied to a store, and make sure the right person gets the applications?' NEW — Public careers page: every open opening ALSO auto-populates one branded public page (joinwingman.app/careers/<slug>) that lists everything they're hiring for, grouped BY LOCATION with a location picker so a visitor at a 20-store group jumps straight to the roles near them, each role with an Apply button pre-set to that role and its ad trimmed to a few lines with a Show more toggle; a keyword search and collapsible per-location sections keep the page short even at 30+ openings so no location gets buried — the single 'here's everything we're hiring for' link to put on their website, Google Business Profile, and social (vs the per-channel /j/ links for tracking a specific board). Close a role and it drops off automatically. NEW — each opening also has its own DIRECT 'Job page' link (the role + full description + Apply button) to share on its own, and any opening can be UNLISTED: kept off the public careers page and out of search while its direct link still works — perfect for a confidential or one-off hire shared only with certain people (toggle it on the opening or via a checkbox when creating/editing). NEW — role-filtered careers link: the careers page has a role filter (and a search that matches the ROLE name/type/pay, not the ad body, so 'manager' returns manager roles — not every ad that mentions a manager), and from the careers bar the operator can copy a link that opens the page pre-filtered to ONE role across all locations (e.g. joinwingman.app/careers/slug?role=Manager) — ideal when running an ad specifically for managers, while the plain link still shows everything. Big hook: every open role gets its own indexable job page with proper JobPosting markup, and they're all in Wingman's sitemap — so their openings are eligible to appear in the GOOGLE JOBS results box, free applicant traffic a small restaurant otherwise never gets (owner submits the sitemap once in Search Console to speed first indexing). Surface it from the 'Your public careers page' bar at the top of Job openings — which also hands them a QR code (window decal / flyer / receipt) and a one-snippet EMBED to drop the whole careers page onto their own website (it resizes to fit and always shows their current open roles, so a filled role drops off everywhere automatically) — same self-serve embed treatment as the application form. NEW — Source tracking on the plain apply link: 'Track where applicants come from' generates a tagged link per channel (?src=craigslist / facebook / instagram / indeed, or a custom tag), every application shows 'via Craigslist' on its row, and a Source filter with per-channel counts appears above the tracker — so the operator sees not just how many each channel sends but, paired with the screening tiers, how GOOD they are (e.g. Facebook sends more volume, Craigslist sends more Strong-fit candidates). NEW — 'Ask about your applicants' (AI): the operator asks their applicant pool anything in plain English ('who has real restaurant experience and can work weekends?', 'top 5 to interview first and why', 'what do my strongest applicants have in common?') and gets a concise answer PLUS a clickable shortlist of the matching people. It reads their form answers, screening answers, AI grades, AND resume text (resumes are auto-extracted to searchable text in the background), grounded strictly in what was submitted so it never invents anyone. Scoped to the active pipeline and the manager's locations. It's the 'imagine dumping every application into ChatGPT and asking' capability, built in — the thing that makes a big applicant pile actually manageable.", problem: "Rounds out the system — they collect applications without paying for a careers page or a job board, every applicant lands in one tracked pipeline instead of a messy inbox, interviews get scheduled and never forgotten (daily reminder), and better people flow straight into the scorecard. Great for the owner drowning in DMs and texts from applicants." },
  { name: "Partners (B2B / Community)", what: "A lightweight relationship CRM for the businesses AROUND the restaurant — offices, schools, gyms, non-profits, the chamber. Managers add contacts (or snap a business card and AI reads it into the form), log every touch (call, email, meeting, booked event/fundraiser with the real revenue it brought in), and Wingman flags any connection gone 30+ days quiet with a 'Needs Follow-up' badge and a default 'Needs Follow-up First' sort. Owners set quarterly goals per store (new contacts, events, fundraisers, connections kept warm), and on the 1st of each month Wingman emails a leadership rollup across all stores plus each manager their own store's metrics and a 'hit list' of who to call this week. Managers see only their assigned store(s); owners see everything. Follow-up tasks scheduled while logging an activity email the manager on the due date. NEW — open any contact (click its name) for a full record: the complete activity timeline, and three things right there — EMAIL the contact from inside Wingman (sends from the rep's name, replies routed to the rep's inbox, and auto-logged to the timeline), ADD dated NOTES that stack as history, and SCHEDULE a reach-out (pick a date + next step; Wingman emails the morning it's due, overdue ones flagged red, mark done when handled). Turns each contact into a mini command center so nobody has to remember who needs a touch.", problem: "Catering, group lunches, private events, happy hours, and fundraisers are massive revenue most restaurants leave on the table because nobody systematically works the neighborhood — relationships start warm and quietly go cold. This turns community outreach from random luck into a measured habit: the team always knows who's fading and who to call, leadership sees which stores are actually building relationships, and every booked event's real dollars are tracked. It's Bounce Back's win-back discipline pointed at local businesses instead of guests." },
  ...
  { name: "Setup checklist (Start Here)", what: "A friendly, self-pacing checklist on the Start Here page: a handful of moves across four phases that build on each other, each milestone checking itself off as they do the work, with a short encouraging weekly email nudging their next move. Deliberately NOT a deadline — nothing is ever 'overdue' (a brand-new owner exploring for a few days should never be scolded by software they just bought); it shows what's done and what's next, at their own pace, and most of it is already seeded for them. Brand-new owners are dropped straight onto this calm checklist instead of an empty dashboard.", problem: "Answers the objection every operator has — 'we bought software before and never used it.' This makes implementation feel easy and guided (Apple-style: sensible defaults, one clear next step) so they actually run it and get a return — without the guilt-trip that makes people abandon a setup flow." },
  ...
  { name: "Doc → test (AI document import)", what: "Upload an existing SOP, ops manual, or menu — PDF or text — and Wingman reads it and builds a full training + auto-scored quiz straight from its contents (owner reviews and approves before it saves). PDFs are read natively; text can be pasted too. NEW — one handbook → MANY roles: 'Import a handbook for all roles' takes ONE handbook that spans several roles and splits it into a training program for every role the restaurant runs in a single pass — the owner reviews each role, unchecks any they don't want, and saves the rest at once (it replaces each role's AI-built items and keeps hand-typed ones). So an operator whose whole operation lives in one binder isn't importing it once per role — it's one upload for the entire team.", problem: "Operators already have their standards written down somewhere — a binder, a PDF, a Google Doc. This turns that dead document into live, testable training in one step, removing the single biggest reason training software never gets used: nobody has time to re-type their manual into it. And when that document covers the whole operation, the whole team's training is built from one upload instead of role-by-role." },
...
export const SYSTEM_REFERENCE: SystemNote[] = [
  ...
  { name: "Lean navigation (progressive disclosure)", detail: "The sidebar defaults to a short, calm list — the sections a restaurant uses most are always visible (Dashboard, Guests, Hiring, Training & Standards, Accountability, Staff, Culture), and everything else is tucked under a single 'More' expander (Shift, Manager updates, Questions, Revenue Growth Planner, Menu Engineering, Experience Audit, Partners, Reporting). 'More' remembers whether it's open and auto-opens when you're on one of those pages, so nothing is ever hard to reach — it's just not all shoved in your face on day one. NEW — the four guest tools (Bounce Back, Reviews, Service Recovery, Journey) are now ONE 'Guests' entry that opens a tabbed surface, instead of four separate destinations — the tabs a person sees respect their access and any per-manager hiding. Hospitality Score lives inside Reporting (one link at the top of that page) rather than as its own nav item. This is the answer to 'isn't there a lot here?' — there's a lot of POWER, but the default view is deliberately simple, and per-manager section hiding (below) trims it further per person. Demo note: point out how short the sidebar is, click 'Guests' to show the tabbed guest surface, then click 'More' to reveal the depth.", },
  { name: "Per-manager dashboard (hide sections)", detail: "Access LEVEL sets the ceiling of what a role can reach; on top of that, an owner can tailor an INDIVIDUAL manager's or shift lead's dashboard by hiding sections they don't need. When inviting or editing a Manager/Shift Lead, a 'Hide sections' checklist ticks off areas (Partners, Menu Engineering, Reporting, Hiring, Audit, etc.) — ticked ones vanish from that one person's sidebar AND are blocked if they deep-link, so they get a clean, focused view built around what they actually run. It's per PERSON, not per role (hide different things for different managers), never affects anyone else's view or the owner's, and is reversible anytime by unticking. Super Admins always see everything; Staff already have a fixed minimal view, so the checklist only appears for Manager/Shift Lead. Answers the very common 'my GM doesn't need to see the community-outreach or menu-numbers stuff — can I just turn it off for them?' — yes, without changing their actual permissions. Demo it in the Edit dialog: tick a couple of sections and point out how the sidebar would shrink for that person." },
  { name: "Corporate / HQ team members", detail: "Not everyone works at a store. When inviting or editing anyone below Super Admin, location access has three choices: All locations, CORPORATE (not tied to a location), or Specific locations. 'Corporate' is for HQ roles — a marketing coordinator, a bookkeeper, an ops director, HR — who aren't based at one restaurant: there's no store to pick (removes the 'location is required' friction), they see every location for the sections they can access, and they show as 'Corporate' on the team list instead of a fake home store. Pair it with per-section hiding to scope them by FUNCTION rather than geography — e.g. a corporate marketer set to Manager + Corporate, then hide everything except Partners and Hiring, so they run outreach and recruiting across all stores and see nothing else. It reuses the same all-locations plumbing under the hood (so it's not a heavier new 'org role' concept) but reads correctly for the customer. Answers 'some of my people are corporate, not restaurant staff — how do I add them?' — set them Corporate and hide what they don't need." },
  ...
  { name: "Direct POS integrations", detail: "Square connects in one click from Settings → API access → Direct integrations (no API key or Zapier). It auto-syncs customers into Guest Bounce Back (deduped by email/phone) and the last 7 days of sales into the weekly Business Health card, matched per location by store name for multi-unit operators — daily, or on-demand with 'Sync now'. Only the account owner can connect/disconnect. Clover connects the same one-click way (each Clover store is its own merchant, so multi-unit operators connect each store). Toast, Lightspeed, and Heartland Retail connectors are built too — Toast links by Restaurant GUID (after enabling Wingman in Toast), Lightspeed connects via one-click OAuth, and Heartland Retail links by pasting a user access token you generate in Heartland (its OAuth registrations are closed, so no redirect) — Wingman auto-discovers your account subdomain and each account can cover multiple locations, matched by name. NEW — SevenRooms (reservations + guest CRM, not a POS) is built too: because it holds guest PROFILES and visit history rather than just sales, it's an especially strong source — it syncs reservation guest profiles straight into Guest Bounce Back (deduped), linked by Venue ID after enabling Wingman as a SevenRooms partner integration. Each partner card shows 'Onboarding in progress' until its credentials go live; Global Payments (Genius POS), Toast, Lightspeed, and SevenRooms are in partner onboarding now, while Heartland Retail is live immediately since it's token-based (no partner credentials needed). The integrations list is alphabetical: Clover, Global Payments, Heartland Retail, Lightspeed, SevenRooms, Square, Toast. A code/Zapier API path also exists for any other POS. (OpenTable is a common ask too: its guest-data API is partnership-gated and not openly available, so the practical path today is importing an OpenTable guest export via the guest CSV import.)" },
