...
export const SYSTEM_REFERENCE: SystemNote[] = [
  ...
  { name: "Menu items (Server & Bartender training)", detail: "Server and Bartender programs have a Menu items section — each dish/drink with description, allergens, a pairing, and an upsell line staff can use. The owner uploads a photo or PDF of their menu and Wingman's AI reads every item and fills all of that in, including a category for each item (it uses the menu's own sections when present and infers one when the menu isn't sectioned). Items then display grouped by category (Appetizers, Salads, Mains, Desserts, Cocktails…) in collapsible sections, so a 50-item menu is easy to scan on a phone mid-shift instead of one endless scroll — the same grouped view staff see. Managing it is easy too: edit any item inline (pencil), multi-select with checkboxes to bulk-delete, and re-upload anytime — Wingman matches by name and updates in place (keeping the owner's popularity/profit numbers) instead of duplicating. Demo the upload-and-auto-fill as the 'we did the data entry for you' moment; flip to Staff View to show how a server actually browses it." },
  { name: "Kitchen recipes (how to make each dish)", detail: "Every dish can carry a Recipe — a numbered, step-by-step 'how to make it' with a real photo on each step (snapped or uploaded from a phone or computer). Managers/owners build and edit it; the kitchen (Chef role) pulls it up on the line so every plate goes out to spec. It hangs off the menu item, so a recipe never gets lost: archiving or 'deleting' a dish keeps its recipe and restores with it. Photos are auto-shrunk in the browser before upload, so it doesn't balloon storage. Coming next: 'turn a recipe into a test' so cooks get quizzed on the steps. For a kitchen-heavy operator, this is the consistency pitch — 'the same plate every cook, every shift.'" },
