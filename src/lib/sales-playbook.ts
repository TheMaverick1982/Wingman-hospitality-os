...
export const PRODUCT_TOUR: ProductArea[] = [
  ...
  { name: "Your language (English / Spanish)", what: "Every person picks their own language on first login (default English) and can switch anytime from a top-bar toggle. The choice is per user, so a Spanish-speaking cook does their training and tests in Spanish while the owner stays in English — starting with the staff test-taking experience, expanding across the app.", problem: "Half the back-of-house often reads English as a second language, so training and tests don't actually land. Letting each person work in their own language removes the excuse and makes standards real for the whole team — a differentiator most restaurant software ignores." },
];
